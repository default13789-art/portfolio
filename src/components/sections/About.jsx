...
const onChainStats = [
  { label: 'Projects Shipped', value: '12+', icon: '⬡', color: '#3ce6f9' },
  { label: 'Commits', value: '2.4K', icon: '◈', color: '#a855f7' },
...
const About = () => {
  ...
  return (
    <section id="about" className="section-padding bg-[#0d0d15] relative overflow-hidden" ref={sectionRef}>
      ...
      {/* Ambient glow orbs */}
      <div className="absolute top-0 left-1/4 w-72 h-72 bg-[#3ce6f9]/6 rounded-full filter blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-72 h-72 bg-[#a855f7]/6 rounded-full filter blur-3xl pointer-events-none" />

      <div className="section-container relative z-10">
        {/* Section Header */}
        <div className={`reveal-section-header ${isVisible ? 'reveal-active' : ''} text-center mb-16`}>
          {/* Web3 section label */}
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-mono bg-[#3ce6f9]/5 border border-[#3ce6f9]/20 text-[#3ce6f9]/70 mb-4">
            <span className="w-1.5 h-1.5 rounded-full bg-[#3ce6f9] animate-pulse" />
            0x02 · IDENTITY BLOCK
          </span>
          <h2 className="heading-lg gradient-text mb-4">About Me</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-[#3ce6f9] to-[#a855f7] mx-auto rounded-full" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className={`reveal reveal-fade-in-left ${isVisible ? 'reveal-active' : ''}`}>
            <div className="relative group max-w-sm mx-auto">
              {/* Outer glow */}
              <div className="absolute -inset-0.5 bg-gradient-to-r from-[#3ce6f9] to-[#a855f7] rounded-2xl blur opacity-25 group-hover:opacity-50 transition-opacity duration-500" />

              {/* Card */}
              <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-[#0d0d15]">
                {/* Card header — NFT collection bar */}
                <div className="flex items-center justify-between px-4 py-2.5 bg-gradient-to-r from-[#3ce6f9]/8 to-[#a855f7]/8 border-b border-white/5">
                  <div className="flex items-center gap-2">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
                      <path d="M12 2L4 12l8 4.8L20 12 12 2z" fill="#3ce6f9" opacity="0.9"/>
                      <path d="M12 18.5L4 13.8l8 8.2 8-8.2-8 4.7z" fill="#a855f7" opacity="0.8"/>
                    </svg>
                ...
                <div className="relative p-8 flex flex-col items-center">
                  ...
                  <div className="relative mb-6">
                    <div className="absolute -inset-2 rounded-full border-2 border-dashed border-[#3ce6f9]/30 animate-spin" style={{ animationDuration: '12s' }} />
                    <div className="absolute -inset-4 rounded-full border border-[#a855f7]/15 animate-spin" style={{ animationDuration: '20s', animationDirection: 'reverse' }} />
                    ...
                    {/* Level badge */}
                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-xs font-bold bg-gradient-to-r from-[#3ce6f9] to-[#a855f7] text-white whitespace-nowrap shadow-lg">
                      LVL 9 · Builder
                    </div>
                  </div>
                  ...
                  <p className="text-sm text-[#3ce6f9]/70 font-mono mb-4">Prompt Engineer · AI Dev</p>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {portfolioData.about.highlights.map((highlight, index) => (
                <div
                  key={index}
                  className={`reveal reveal-fade-in-up reveal-stagger-${Math.min(index + 2, 10)} ${isVisible ? 'reveal-active' : ''} flex items-center gap-2.5 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] hover:border-[#3ce6f9]/20 transition-colors`}
                >
            ...
            <div className={`reveal reveal-fade-in-up reveal-stagger-4 ${isVisible ? 'reveal-active' : ''}`}>
              <div className="p-4 rounded-xl bg-[#0a0a0f] border border-white/[0.06]">
                ...
                <div className="space-y-2">
                  {[
                    { action: 'Deployed', target: 'AI Automation Pipeline', time: '2h ago', color: '#3ce6f9' },
                    { action: 'Merged PR', target: 'Neural UI Component', time: '1d ago', color: '#a855f7' },
