...
export const GlowFrame = ({ children, className = '', color = 'cyan' }) => {
  const colors = {
    cyan: 'from-[#00F5FF] to-[#8A2BE2]',
    purple: 'from-[#8A2BE2] to-[#8A2BE2]',
    green: 'from-[#22c55e] to-[#00F5FF]'
  };

  return (
    <div className={`relative p-[1px] rounded-lg bg-gradient-to-r ${colors[color]} ${className}`}>
      <div className="relative bg-[#050510] rounded-lg">
        {children}
      </div>
    </div>
  );
  ...
  <div className="text-center mb-16">
    ...
    {badge && (
      <div className="inline-flex items-center gap-2 px-4 py-2 mb-6 glass-card rounded-full">
        ...
        <span className="text-sm font-mono text-[#00F5FF]">{badge}</span>
      </div>
    )}
    ...
    <div className="flex items-center justify-center gap-4 mt-8">
      <div className="h-px w-16 bg-gradient-to-r from-transparent to-[#00F5FF]" />
      <div className="w-2 h-2 bg-[#00F5FF] rounded-full animate-pulse" />
      <div className="h-px w-16 bg-gradient-to-l from-transparent to-[#8A2BE2]" />
    </div>
  </div>
);
...
export const HoverCard = ({
  ...
  >
    ...
    <div className="absolute inset-0 overflow-hidden rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#00F5FF]/5 to-transparent translate-y-[-100%] group-hover:translate-y-[100%] transition-transform duration-1000" />
    </div>
...
export const TechTooltip = ({ children, content }) => (
  <div className="group relative inline-block">
    ...
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 glass-card rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap">
      <div className="text-xs font-mono text-[#00F5FF]">{content}</div>
      {/* Arrow */}
      <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-[#050510]/80" />
    </div>
  </div>
);
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-sm">
        ...
        <span className="font-mono text-[#00F5FF]">{progress}%</span>
      </div>
...
export const TechTag = ({ children, variant = 'default', size = 'md' }) => {
  const variants = {
    default: 'bg-[#00F5FF]/10 border-[#00F5FF]/30 text-[#00F5FF]',
    purple: 'bg-[#8A2BE2]/10 border-[#8A2BE2]/30 text-[#8A2BE2]',
...
export const CornerAccent = ({ position = 'top-left' }) => {
  ...
  return (
    <div
      className={`absolute w-8 h-8 border-[#00F5FF]/30 ${positions[position]}`}
      style={{
